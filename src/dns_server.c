	...
	while (true) {
		...
		hash_for_each_safe(request->ip_map, bucket, tmp, addr_map, node)
		{
			...
			if (addr_map->addr_type == DNS_T_A) {
				...
				context->ip_num++;
				tlog(TLOG_INFO, "result: %s, rcode: %d, index: %d, %d.%d.%d.%d\n", request->domain, request->rcode,
					 context->ip_num, addr_map->ipv4_addr[0], addr_map->ipv4_addr[1], addr_map->ipv4_addr[2],
				...
				tlog(TLOG_INFO,
					 ...
					 request->domain, request->rcode, context->ip_num, request->ipv6_addr[0], addr_map->ipv6_addr[1],
					 addr_map->ipv6_addr[2], addr_map->ipv6_addr[3], addr_map->ipv6_addr[4], addr_map->ipv6_addr[5],
					 addr_map->ipv6_addr[6], addr_map->ipv6_addr[7], addr_map->ipv6_addr[8], addr_map->ipv6_addr[9],
					 addr_map->ipv6_addr[10], addr_map->ipv6_addr[11], addr_map->ipv6_addr[12], addr_map->ipv6_addr[13],
					 addr_map->ipv6_addr[14], addr_map->ipv6_addr[15]);
			}
		}
		pthread_mutex_unlock(&request->ip_map_lock);
...
static int _dns_add_rrs(struct dns_server_post_context *context)
{
	...
	if (request->has_ipv4 && context->qtype == DNS_T_A && context->do_force_soa == 0) {
		...
		context->ip_num++;
		tlog(TLOG_INFO, "result: %s, rcode: %d, index: %d, %d.%d.%d.%d\n", request->domain, request->rcode,
			 context->ip_num, request->ipv4_addr[0], request->ipv4_addr[1], request->ipv4_addr[2],
			 request->ipv4_addr[3]);
	}
